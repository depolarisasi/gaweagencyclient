...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Paket Langganan</h1>
                <p class="text-gray-600">Edit paket langganan: {{ $subscriptionPlan->name }}</p>
            </div>
        </div>

        <!-- Form -->
        <div class="bg-white rounded-lg shadow">
            <form method="POST" action="{{ route('admin.subscription-plans.update', $subscriptionPlan) }}" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price" value="{{ old('price', $subscriptionPlan->price) }}" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="billing_cycle" class="select select-bordered w-full @error('billing_cycle') select-error @enderror">
                            ...
                            <option value="monthly" {{ old('billing_cycle', $subscriptionPlan->billing_cycle) === 'monthly' ? 'selected' : '' }}>Bulanan</option>
                            <option value="6_months" {{ old('billing_cycle', $subscriptionPlan->billing_cycle) === '6_months' ? 'selected' : '' }}>Semester (6 Bulan)</option>
                            <option value="annually" {{ old('billing_cycle', $subscriptionPlan->billing_cycle) === 'annually' ? 'selected' : '' }}>Tahunan (12 Bulan)</option>
                            <option value="2_years" {{ old('billing_cycle', $subscriptionPlan->billing_cycle) === '2_years' ? 'selected' : '' }}>2 Tahunan (24 Bulan)</option>
                            <option value="3_years" {{ old('billing_cycle', $subscriptionPlan->billing_cycle) === '3_years' ? 'selected' : '' }}>3 Tahunan (36 Bulan)</option>
                        </select>
                        @error('billing_cycle')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="cycle_months" value="{{ old('cycle_months', $subscriptionPlan->cycle_months) }}" 
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" 
                              ...
                              placeholder="Deskripsi singkat tentang paket ini...">{{ old('description', $subscriptionPlan->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
